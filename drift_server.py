...
import threading, time, os, random, argparse, json, requests
...
parser.add_argument("--device-id", type=str, default=os.getenv("DEVICE_ID", "linealert-test-device"))
parser.add_argument("--alert-webhook", type=str, default=os.getenv("ALERT_WEBHOOK", "http://localhost:1880/linealert-drift"))
...
def log_lasnap(alert_level, alert_message, current_value):
    snapshot = {
    ...
    filename = f"snap_{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}.lasnap"
    path = os.path.join(DATA_DIR, filename)
    with open(path, "w") as f:
        json.dump(snapshot, f, indent=2)
    print(f"[{alert_level}] Logged .lasnap to {path}")

    try:
        requests.post(ALERT_WEBHOOK, json=snapshot, timeout=1)
        print(f"[{alert_level}] Alert POSTed to {ALERT_WEBHOOK}")
    except Exception as e:
        print(f"[{alert_level}] Failed to POST alert: {e}")
    ...
    while True:
        ...
        with lock:
            ...
            if alert_level in ["YELLOW", "RED"]:
                log_lasnap(alert_level, alert_message, value)
